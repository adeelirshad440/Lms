...
def save_subject_image(instance, filename):
    ...
    if instance.user.username:
        filename = 'Subject_pictures/{}{}'.format(
            instance.subject_id, ext)
...
def save_lesson_files(instance, filename):
    ...
    if instance.lesson_id:
        ...
        if os.path.exists(filename):
            new_ame = str(instance.lesson.lesson_id) + str('1')
            filename = 'lesson_images/{}/{}.{}'.format(
                instance.lesson_id, new_ame, ext)
    return os.path.join(upload_to, filename)
...
class Lesson (models.Model):
    ...
    ppt = models.FileField(upload_to=save_lesson_files,
                           verbose_name='Presention', blank=True)
...
class Comment(models.Model):
    ...
    def save(self, *args, **kwargs):
        self.comm_name = slugify(
            'comment by ' + '-' + str(self.author) + str(self.data_added))
        super().save(*args, **kwargs)
